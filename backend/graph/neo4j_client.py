...
class Neo4jClient:
    ...
    def find_node(self, label: str, name: str, tenant_id: str = None) -> Optional[Dict[str, Any]]:
        ...
        query = f"MATCH (n:{safe_label} {{name: $name}}) WHERE true {tenant_filter} RETURN n"
        params = {"name": name}
        if tenant_id:
    ...
    def fulltext_search(self, search_term: str, tenant_id: str = None) -> List[Dict[str, Any]]:
        ...
        WHERE (
            (n.name IS NOT NULL AND (
                replace(replace(toLower($term), '-', ''), ' ', '') CONTAINS replace(replace(toLower(n.name), '-', ''), ' ', '') OR
                replace(replace(toLower(n.name), '-', ''), ' ', '') CONTAINS replace(replace(toLower($term), '-', ''), ' ', '')
            ))
           OR (n.description IS NOT NULL AND (
                replace(replace(toLower($term), '-', ''), ' ', '') CONTAINS replace(replace(toLower(n.description), '-', ''), ' ', '') OR
                replace(replace(toLower(n.description), '-', ''), ' ', '') CONTAINS replace(replace(toLower($term), '-', ''), ' ', '')
           ))
        ) {tenant_filter}
    ...
    # --- Phase 6: Temporal Graph Queries ---

    def get_temporal_subgraph(self, timestamp: str, limit: int = 100, tenant_id: str = None) -> Dict[str, Any]:
        """
        Retrieves nodes and edges that were valid or occurred at/before the given timestamp.
        """
        tenant_filter = "AND n.tenant_id = $tenant_id AND m.tenant_id = $tenant_id" if tenant_id else ""
        # Match nodes and relationships valid at the timestamp
        query = f"""
        MATCH (n)-[r]-(m)
        WHERE (n.valid_from IS NULL OR n.valid_from <= $timestamp)
        ...
        try:
            ...
            for row in results:
                ...
                n_name = n.get("name")
                if n_name:
                    nodes_dict[n_name] = dict(n)
                
                m_name = m.get("name")
                if m_name:
                    nodes_dict[m_name] = dict(m)
                
                if n_name and m_name:
                    rel_type = r[1] if isinstance(r, tuple) else type(r).__name__
                    if hasattr(r, 'type'): rel_type = r.type
                    edge_key = (min(n_name, m_name), max(n_name, m_name), rel_type)
                    edges_dict[edge_key] = {
                        "source": n_name,
                        "target": m_name,
                        "type": rel_type,
                        "event_time": dict(r).get("event_time")
                    }
        except Exception as e:
            logger.error(f"Error fetching temporal subgraph: {{e}}")
    ...
    def get_incident_timeline(self, equipment_name: str, tenant_id: str = None) -> List[Dict[str, Any]]:
        """
        Retrieves a chronological timeline of events (Incidents, Maintenance) for a piece of equipment.
        """
        tenant_filter = "AND e.tenant_id = $tenant_id AND evt.tenant_id = $tenant_id" if tenant_id else ""
        query = f"""
        MATCH (e:Equipment {{name: $name}})<-[r:OCCURRED_ON|MAINTAINED_BY|AFFECTED_BY|INSPECTED_BY]-(evt)
        WHERE (evt.event_time IS NOT NULL OR r.event_time IS NOT NULL) {tenant_filter}
        RETURN labels(evt) AS evt_labels, evt.name AS evt_name, type(r) AS rel_type,
               coalesce(evt.event_time, r.event_time) AS event_time,
               evt.description AS description, r.evidence AS evidence
        ORDER BY event_time DESC
        LIMIT 50
        """
        params = {"name": equipment_name}
        if tenant_id:
            params["tenant_id"] = tenant_id
        ...
        try:
            ...
            for row in results:
                timeline.append({{
                    "event_type": row["evt_labels"][0] if row["evt_labels"] else "Event",
            ...
            logger.error(f"Error fetching incident timeline: {{e}}")
    ...
    def get_subgraph_by_labels(self, labels: List[str], limit: int = 500, tenant_id: str = None) -> Dict[str, Any]:
        ...
        safe_labels = []
        for l in labels:
            safe_l = re.sub(r'[^a-zA-Z0-9_]', '', l)
            if safe_l:
                safe_labels.append(safe_l)
                
        if not safe_labels:
            return {"nodes": [], "edges": []}

        label_clause = " OR ".join([f"'{l}' IN labels(n)" for l in safe_labels])
        tenant_filter = "AND n.tenant_id = $tenant_id" if tenant_id else ""
    ...
    def search_nodes(self, search_term: str, limit: int = 20, tenant_id: str = None) -> List[Dict[str, Any]]:
        ...
        query = f"""
        MATCH (n)
        WHERE n.name IS NOT NULL AND replace(replace(toLower(n.name), '-', ''), ' ', '') CONTAINS replace(replace(toLower($term), '-', ''), ' ', '') {tenant_filter}
    ...
    def get_all_engineers(
        ...
        WHERE (sa:Asset OR sa:Equipment) AND sa.tenant_id = $tenant_id
          AND NOT EXISTS { 
              MATCH (sa)<-[:MAINTAINED_BY|INSPECTED_BY|OPERATED_BY|KNOWLEDGE_OWNER|OWNED_BY|KNOWLEDGE_OWNER_FOR]-(other) 
              WHERE other.tenant_id = $tenant_id 
                AND (other.type = 'Person' OR other.entity_type = 'Person' OR 'Person' IN labels(other)) 
                AND id(other) <> id(p) 
          }
...
# Singleton instance
    ...
    def bulk_upsert(self, tenant_id: str, entities: list[dict], relationships: list[dict]) -> dict:
        ...
        from backend.config import get_settings
        settings = get_settings()
        batch_size = getattr(settings, 'NEO4J_BATCH_SIZE', 500)
        
        total_nodes = 0
        total_edges = 0
        
        with self.get_driver().session() as session:
            for i in range(0, max(len(entities), len(relationships)), batch_size):
                e_batch = entities[i:i+batch_size]
                r_batch = relationships[i:i+batch_size]
                n, e = session.execute_write(_tx, e_batch, r_batch)
                total_nodes += n
                total_edges += e
                
        return {"nodes_upserted": total_nodes, "edges_upserted": total_edges}
